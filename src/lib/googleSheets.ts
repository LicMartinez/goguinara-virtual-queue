...
class GoogleSheetsService {
  ...
  async appendRowToSheet(data: SheetData): Promise<any> {
    try {
      const range = 'Hoja 1!A:E'; // Rango expandido para soportar más filas
  ...
  async getQueueData(): Promise<QueueEntry[]> {
    try {
      // Expandir rango para soportar hasta 1000 filas (más que suficiente para 50+ personas)
  ...
  async updateStatus(rowIndex: number, newStatus: string): Promise<void> {
    try {
      const range = `Hoja 1!D${rowIndex + 2}`; // +2 because of header and 0-based index
  ...
  async getAverageWaitTime(): Promise<number> {
    try {
      // Intentar obtener el tiempo promedio desde una celda específica (G1)
      const response = await this.sheets.spreadsheets.values.get({
        spreadsheetId: this.spreadsheetId,
        range: 'Hoja 1!G1',
      });
      ...
      if (response.data.values && response.data.values[0] && response.data.values[0][0]) {
        ...
        return isNaN(waitTime) ? 5 : waitTime; // Default 5 minutos si no es válido
      }
  ...
  async initializeSheet(): Promise<void> {
    try {
      ...
      if (!response.data.values || response.data.values.length === 0) {
        ...
        // Asegurar que existe el tiempo promedio por defecto
